...
int main() {
    ...
    exitGame(fp); // Check if user wants to exit before starting
    mode = tryMode(fp);  // mode selection
    exitGame(fp); // Check if user wants to exit after selecting mode
    size = trySize(fp);  // grid size selection
